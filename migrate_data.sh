...
echo ""
echo "Create dumps..."
ssh guavapi "psql -U postgres -h localhost -d h64 -c \"COPY (SELECT * FROM heat_pump_metrics_electric_power_pulse) TO '/usb-data/h64_old/h64_heat_pump.csv'  WITH DELIMITER ',' CSV HEADER; COPY (SELECT time, total_load_power, total_grid_power, load_power_l1, load_power_l2, load_power_l3, pv1_power, pv2_power, battery_power FROM inverter_metrics_faster) TO '/usb-data/h64_old/h64_inverter_faster.csv'  WITH DELIMITER ',' CSV HEADER; COPY (SELECT * FROM inverter_metrics_slow) TO '/usb-data/h64_old/h64_inverter_slow.csv'  WITH DELIMITER ',' CSV HEADER; COPY (SELECT * FROM weather_station_metrics) TO '/usb-data/h64_old/h64_weather_station.csv'  WITH DELIMITER ',' CSV HEADER;\""
