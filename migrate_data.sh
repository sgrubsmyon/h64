...
ssh guavapi "psql -U postgres -h localhost -d h64 -c \"COPY (SELECT * FROM heat_pump_metrics_electric_power_pulse) TO '/tmp/h64_heat_pump.csv'  WITH DELIMITER ',' CSV HEADER;\""
ssh guavapi "psql -U postgres -h localhost -d h64 -c \"COPY (SELECT time, total_load_power, total_grid_power, load_power_l1, load_power_l2, load_power_l3, pv1_power, pv2_power, battery_power FROM inverter_metrics_faster) TO '/tmp/h64_inverter_faster.csv'  WITH DELIMITER ',' CSV HEADER;\""
ssh guavapi "psql -U postgres -h localhost -d h64 -c \"COPY (SELECT * FROM inverter_metrics_slow) TO '/tmp/h64_inverter_slow.csv'  WITH DELIMITER ',' CSV HEADER;\""
ssh guavapi "psql -U postgres -h localhost -d h64 -c \"COPY (SELECT * FROM weather_station_metrics) TO '/tmp/h64_weather_station.csv'  WITH DELIMITER ',' CSV HEADER;\""
# Copy dumps over
rsync -rtlPvi guavapi:/tmp/h64_heat_pump.csv /tmp/h64_heat_pump.csv
rsync -rtlPvi guavapi:/tmp/h64_inverter_faster.csv /tmp/h64_inverter_faster.csv
rsync -rtlPvi guavapi:/tmp/h64_inverter_slow.csv /tmp/h64_inverter_slow.csv
rsync -rtlPvi guavapi:/tmp/h64_weather_station.csv /tmp/h64_weather_station.csv
# Insert dumps into temprary database
